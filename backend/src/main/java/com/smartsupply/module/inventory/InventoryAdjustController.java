...
public class InventoryAdjustController {
    ...
    public Result<Void> adjust(@RequestBody Map<String, Object> body) {
        ...
        // 更新库存（不存在则新建）
        List<Map<String, Object>> inv = jdbc.queryForList("SELECT id, quantity FROM inventory WHERE sku_id=? AND warehouse_id=?", skuId, warehouseId);
        if (inv.isEmpty()) {
            if (changeQty < 0) return Result.fail(400, "该仓库暂无此SKU，无法扣减");
            jdbc.update("INSERT INTO inventory(sku_id, warehouse_id, quantity) VALUES (?,?,?)", skuId, warehouseId, changeQty);
        } else {
            long cur = ((Number) inv.get(0).get("quantity")).longValue();
            long next = cur + changeQty;
            if (next < 0) return Result.fail(400, "扣减后库存不能为负，当前库存" + cur);
            jdbc.update("UPDATE inventory SET quantity=?, updated_at=now() WHERE sku_id=? AND warehouse_id=?", next, skuId, warehouseId);
        }
