...
public class PurchaseTools {
    ...
    private final PurchaseOrderWriter orderWriter;
    ...
    public PurchaseTools(JdbcTemplate jdbc, IdempotencyService idempotency, ToolSecurity security, ObservationService observation,
                         PurchaseOrderWriter orderWriter) {
    ...
    public Map<String, Object> createPurchaseOrder(
        ...
        try {
            ...
            // 参数与存在性校验先行，幂等 key 只在"确定要创建"时占用：
            // 此前校验前就占 key 且失败不释放，一次校验失败会烧掉合法重试通道十分钟
            ...
            idemKey = "idem:po:" + CurrentUser.username() + ":" + supplierId + ":" + skuCode + ":" + quantity + ":" + unitPrice;
            ...
            String remark = "由Agent创建 操作人:" + actor + (below ? " [低于安全库存]" : " [库存校验通过]");
            Long orderId = orderWriter.persist(orderNo, supplierId, amount, remark, actor, skuId, quantity, unitPrice);
            ok = true;
            return Map.of("success", true, "orderNo", orderNo, "orderId", orderId, "totalAmount", amount, "belowSafety", below);
        } catch (Exception e) {
            // 本次尝试未创建任何数据（写路径在独立事务里，异常即回滚），释放幂等 key 让合法重试通过
            if (idemKey != null) idempotency.release(idemKey);
            throw e;
        } finally {
