...
def _to_int(name: str, default: int) -> int:
    """数值环境变量解析保护：垃圾值回退默认并告警——此前 int(env) 访问即抛 ValueError，
    一个写错的 env 会在首次访问配置处炸掉整个请求路径。"""
    try:
        return int(os.getenv(name, "") or default)
    except (TypeError, ValueError):
        import logging
        logging.getLogger(__name__).warning("环境变量 %s=%r 不是合法整数，回退默认 %s", name, os.getenv(name), default)
        return default
...
def _to_float(name: str, default: float) -> float:
    try:
        ...
        import logging
        logging.getLogger(__name__).warning("环境变量 %s=%r 不是合法数字，回退默认 %s", name, os.getenv(name), default)
        return default
...
def _getattr_impl(name: str):
    ...
    if name == "RUN_TOKEN_BUDGET":
        ...
        return _to_int("RUN_TOKEN_BUDGET", 0)
    ...
    if name == "LLM_MAX_ATTEMPTS":
        return _to_int("LLM_MAX_ATTEMPTS", 3)
    if name == "LLM_BACKOFF_SECONDS":
        return _to_float("LLM_BACKOFF_SECONDS", 0.8)
    if name == "LLM_TIMEOUT_SECONDS":
        return _to_float("LLM_TIMEOUT_SECONDS", 240.0)
    # ReAct 最大迭代轮数：此前硬编码 6，轮数与成本/延迟直接相关，应可按环境调整
    if name == "MAX_ITERATIONS":
        return _to_int("MAX_ITERATIONS", 6)
    ...
    if name == "RUN_TIMEOUT_SECONDS":
        return _to_float("RUN_TIMEOUT_SECONDS", 360.0)
